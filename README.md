...
- **主要类及方法**：
    - `RaspiInfo`类：
        ...
        - `get_cpu_usage`方法：用于获取树莓派CPU的使用率，通过获取一段时间平均负载并转换为整数百分比后返回，若获取过程中出现异常则打印相应错误提示信息并返回`None`。
...
- **主要类及方法**：
    - `MagnetometerReader`类：
        - 在`__init__`方法中，接收`config_manager`作为参数，初始化一系列与串口相关的实例属性（如`serial_port`、`baudrate`等）以及`serial_connection`（初始化为`None`），并调用`initialize_serial`方法自动初始化串口。
